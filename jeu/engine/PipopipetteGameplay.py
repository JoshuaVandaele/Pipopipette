...
class PipopipetteGameplay:
    ...
    def __init__(
        self: PipopipetteGameplay,
        list_player_name: list[str],
        pipopipette: Pipopipette = Pipopipette(),
    ) -> None:
        """
        Args:
        ...
        """
        self.__LIST_PLAYER: Final[list[Player]] = [
            Player(list_player_name[i], i) for i in range(len(list_player_name))
        ]
        self.__pipopipette: Pipopipette = pipopipette
        self.__current_player_ID: int = (
            self.__LIST_PLAYER[0].ID if len(self.__LIST_PLAYER) > 0 else 0
        )

    # <----- getter ----->

    @property
    def LIST_PLAYER(self: PipopipetteGameplay) -> list[Player]:
        return self.__LIST_PLAYER

    @property
    def pipopipette(self: PipopipetteGameplay) -> Pipopipette:
        return self.__pipopipette

    @property
    def current_player_ID(self: PipopipetteGameplay) -> int:
        return self.__current_player_ID

    # <----- setter ----->

    @pipopipette.setter
    def pipopipette(self: PipopipetteGameplay, pipopipette: Pipopipette) -> None:
        self.__pipopipette = pipopipette

    @current_player_ID.setter
    def current_player_ID(self: PipopipetteGameplay, current_player_ID) -> None:
    ...
    def next_player(self: PipopipetteGameplay) -> None:
        ...
        if self.__current_player_ID % len(self.__LIST_PLAYER) == 0:
    ...
    def game_state_string(self) -> str:
        ...
        """
        return "".join(
            str(square.square_owner) for square in self.pipopipette.list_square
        )
